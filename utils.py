...
def sumPatch(x,k):
    ...
    weight = weight.float()  # 将权重转换为 FloatTensor
    x = x.float()  # 将输入转换为 FloatTensor
...
def get_train_images_auto(pre, paths, height=256, width=256, mode='RGB', isPng = False):
    if isinstance(paths, str):
        paths = [paths]
    images = []
    for path in paths:
        if isPng == True:
            image = get_image(pre+"/"+path[:-4]+".png", height, width, mode=mode)
        else:
            image = get_image(pre+"/"+path, height, width, mode=mode)
        if mode == 'L':
            image = np.reshape(image, [1, image.shape[0], image.shape[1]])
        else:
            image = np.reshape(image, [image.shape[2], image.shape[0], image.shape[1]])
